(function() {
    ...
    //     monthartworkTemplate = Handlebars.compile(monthgenresSource),
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    for (i = 0; i < response.items.length; i++) {
                        artists.push(
                            response.items[i].name console.log(response.items[i])
                        )
                        album_art.push(
                            response.items[i].images[2].url
                        )
                    }
                    var data = [];
                    for (i = 0; i < response.items.length; i++) {
                    ...
                    artworkPlaceholder.innerHTML = artworkTemplate(album_art.slice(0, 25)) s
                }
            });
            // Fetch last month
            $.ajax({
                ...
                success: function(response) {
                    ...
                    monthartistsPlaceholder.innerHTML = monthartistsTemplate(artists)
                    monthgenresPlaceholder.innerHTML = monthgenresTemplate(result.slice(0, 100)); //this 100 is the number of genres shown
                    //monthartworkPlaceholder.innerHTML = monthartworkTemplate(album_art)
                }
            });
            // Fetch all time
            $.ajax({
                ...
                success: function(response) {
                    ...
                    alltimeartistsPlaceholder.innerHTML = alltimeartistsTemplate(artists)
                    alltimegenresPlaceholder.innerHTML = alltimegenresTemplate(result.slice(0, 100)); //this 100 is the number of genres shown
                    //alltimeartworkPlaceholder.innerHTML = alltimeartworkTemplate(album_art)
                }
            });
        } else {
